...
export function renderHome(state) {
  ...
  return `
    <div class="relative" style="height:calc(100dvh - 4rem)">
        ...
        <div class="flex gap-2">
          <div class="flex-1 relative">
            <input
              ...
              value="${searchLabel}"
              class="w-full pl-10 pr-10 py-3 rounded-xl bg-dark-primary/60 backdrop-blur-xl border border-white/10 text-white placeholder-slate-400 focus:border-primary-500 focus:ring-2 focus:ring-primary-500/20 transition-all shadow-lg"
          ...
          <button
            onclick="openFilters()"
            class="px-3 py-3 rounded-xl bg-dark-primary/60 backdrop-blur-xl border border-white/10 text-slate-400 hover:text-white hover:border-primary-500/50 transition-all shadow-lg"
      ...
      ${spotCount > 0 ? `
        <div class="absolute top-[4.5rem] left-3 z-20 px-3 py-1.5 rounded-full bg-dark-primary/60 backdrop-blur-xl border border-white/10 text-sm shadow-lg">
      ...
      <div class="absolute top-[4.5rem] right-3 z-20 flex flex-col gap-1.5">
        <button
          onclick="homeZoomIn()"
      ...
      <div id="nearby-spots-sheet" class="absolute ${isSplit ? 'hidden' : ''} bottom-20 left-0 right-0 z-20 px-3">
        <!-- Handle bar -->
      ...
      <button
        onclick="openAddSpot()"
        class="fixed bottom-32 right-4 z-30 w-14 h-14 rounded-full bg-primary-500 text-white shadow-lg shadow-primary-500/30 flex items-center justify-center text-xl hover:bg-primary-600 hover:scale-110 transition-all"
